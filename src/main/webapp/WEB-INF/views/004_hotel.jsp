...
<div class="container-fluid d-flex justify-content-end navbar2">
	...
	<div class="topm_hospital "  style="height:2700px;">
		 ...
		 <div class="do_nav title_hospital">
		 	<img  class="photo_hospital" src="./resources/pic/hotel.png">동물 호텔 예약
		 </div>
		 	
		 <div class="displayco mt-4 " >
		<c:forEach var="imsi" items="${hotalist}">
		<div class="displays box_hospital m-3 ">				
		 <a  href="${pageContext.request.contextPath}/detailhotel.do?htname=${imsi.htname}">
			<img class="photo_hospital2"  src="./resources/hotpic/${imsi.htsoo}.jpg" >
		</a>
		...
		<div class="displayc" style="margin-left:30px;">
			...
			<div class="displays">	
				 ...
				 <div class="box2_hospital">${imsi.htaddress2}</div>
				 <div class="box2_hospital">${imsi.httime}</div>
				 
			 </div>	
			 <a class=" subtitle_hospital" href="${pageContext.request.contextPath}/detailhotel.do?htname=${imsi.htname}" style="font-weight:bold;">
			 		[동물 호텔 예약 서비스]  ${imsi.htname}</a>
			 <div class="text_hospital">  ${imsi.htarea}|   | ${imsi.htname} </div>			  
			  	<img class="photo_hospital3"  src="./resources/pic/005_star.png">(4.5)
			  <div class="text2_hospital"> 펫토피아 커뮤니티 인기 호텔 🔥</div>	
			</div>		
			<hr>
		</div>
		  </c:forEach>
		       <div class="pagination justify-content-center" style="margin-top: 30px;">
			      <button  class="btn btn-outline-secondary mx-1" onclick="onPage('first')"> << </button>
			      <button  class="btn btn-outline-secondary mx-1" onclick="onPage('prev')"> < </button>
			    
			    <c:forEach begin="1" end="${totalPages}" var="pageNumber">
			           <button class="btn btn-outline-secondary mx-1" onclick="onPage(${pageNumber})" value="${pageNumber}">${pageNumber}</button>
			    </c:forEach>			    
			     <button class="btn btn-outline-secondary mx-1" onclick="onPage('next')"> > </button>
...
<!-- ======================================================== -->
   ...
   function onPage(type) {
      ...
      // 페이지 이동
      location = 'hotel.do?pageNumber=' + num;
   }
</script>
